...
class MegSpider(scrapy.Spider):
    ...
    def parse(self, response):
        self.crawler.stats.set_value('spiderlog/source_name', u'CCF-MEG-市场周报')
    ...
    def login_callback(self, response):
        ...
        page_count = 3
        index_pages = ['http://www.ccf.com.cn/newscenter/index.php?cur_row_pos=0&cur_pg_num=%s&Class_ID=160000' % i for i in range(3,page_count+1)]
    ...
    def parse_index_page(self, response):
        for link in response.css('ul.newslist li a'):
            ...
            if u'CCF MEG市场周报' in link_title:
                item = LinkItem()
    ...
    def parse_content(self, response):
        ...
        if response.css('td#fontzoom h2[class="lv_two_t"]'):
            ...
            table4 = response.css('td#fontzoom table[class="tb_2017v"]')[5]
            table4_rows = [(row, row.xpath('./td')) for row in table4.xpath('./tbody/tr')]
            span1 = response.css('td#fontzoom p[style*="TEXT-INDENT: 2em"]')[11]
        # 2017-01-01 之前
        else:
        ...
        table_title = u'MEG华东港口库存情况(万吨)（完全统计）'
        for row in table4_rows[1:]:
            yield CellItem(
                report_title=report_title,
                report_date=report_date,
                ...
                index_name=''.join(row[1][0].xpath('.//text()').extract()),
                value_unit='',
                column_name=''.join(table4.xpath('./tbody/tr[1]/td[3]//text()').extract()),
                column_value=''.join(row[1][2].xpath('.//text()').extract()),
            )
        #取港口库存情况数据
        yield CellItem(
            ...
            value_unit='',
            column_name='库存情况',
            column_value=''.join(span1.xpath('.//span//text()').extract()),
        )
