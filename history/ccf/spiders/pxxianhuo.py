    ...
    def parse_content(self, response):
        ...
        for row in table1.xpath('./tbody/tr'):
            ...
            # yield CellItem(
                        # report_title = report_title,
                        # table_title = u'合同价格', 
                        # index_name = row.xpath('./td[1]/text()').extract_first(),
                        # column_name = ''.join(table1.xpath('./thead/tr[1]/td[3]//text()').extract()),
            ...
            # yield CellItem(
                        ...
                        # table_title = u'合同价格', 
                        # index_name = row.xpath('./td[1]/text()').extract_first(),
                        # column_name = ''.join(table1.xpath('./thead/tr[1]/td[4]//text()').extract()),
        ...
        for row in table2.xpath('./tbody/tr'):
            ...
            # yield CellItem(
                        ...
                        # table_title = u'负荷指数', 
                        # index_name = row.xpath('./td[1]/text()').extract_first(),
                        # column_name = ''.join(table2.xpath('./thead/tr[1]/td[3]//text()').extract()),
                        # column_value = ''.join(row.xpath('./td[3]/text()').extract()),
                        # report_date = report_date,
                    # )
            # yield CellItem(
                        ...
                        # table_title = u'负荷指数', 
                        # index_name = row.xpath('./td[1]/text()').extract_first(),
                        # column_name = ''.join(table2.xpath('./thead/tr[1]/td[4]//text()').extract()),
                        # column_value = ''.join(row.xpath('./td[4]/text()').extract()),
                        # report_date = report_date,
                    # )
