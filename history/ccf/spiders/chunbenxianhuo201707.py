...
class Chunbenxianhuo201707Spider(scrapy.Spider):
    name = "chunbenxianhuo201707"
    ...
    def start_requests(self):
        if datetime.datetime.now().hour < 15 or datetime.datetime.now().hour > 16:
    ...
    def login_callback(self, response):
        ...
        page_count = 200
        index_pages = ['http://www.ccf.com.cn/newscenter/index.php?cur_row_pos=0&cur_pg_num=%s&Class_ID=160000' % i for i in range(2,page_count+1)]
    ...
    def parse_content(self, response):
        ...
        if response.css('td#fontzoom h2[class="lv_two_t"]') or response.css('td#fontzoom h4[class="at_title ar_weektitle"]'):#at_title ar_weektitle 35 lv_two_t
            table1 = response.css('td#fontzoom table')[0] #[class="tb_2017v"]
            table1_rows = [(row, row.xpath('./td')) for row in table1.xpath('./tbody/tr')]
            table1_rows = self.normalize_rows(table1_rows)
            table2 = response.css('td#fontzoom table')[1]
            table2_rows = [(row, row.xpath('./td')) for row in table2.xpath('./tbody/tr')]
            table2_rows = self.normalize_rows(table2_rows)
            table3 = response.css('td#fontzoom table')[2]
            table3_rows = [(row, row.xpath('./td')) for row in table3.xpath('./tbody/tr')]
            table4 = response.css('td#fontzoom table')[3]
